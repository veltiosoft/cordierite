...
- ページ実装は **1 テンプレート + locale 引数**（例: `src/pages/index.astro` と `src/pages/ja/index.astro` が同じ `LandingPage` コンポーネントを `locale` 付きで呼ぶ）
...
- [ ] `site/` で `npm run build` が成功し、`dist/` に静的成果物が出力される
...
- [ ] 英語 LP と日本語 LP が同一コンポーネント（または同一テンプレート）から描画される
